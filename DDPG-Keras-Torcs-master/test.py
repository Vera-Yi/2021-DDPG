...
from OU import OU   #Ornstein-Uhlenbeck Process, use to add noise

state_size = 29
action_size = 3  #Steering/Acceleration/Brake
...
optimizer_actor = torch.optim.Adam(actor.parameters(), lr=LRA) #Adam algorithm:Adapative Moment Estimation, an optimization algorithm
...
for i in range(2000):
    ...
    if np.mod(i, 3) == 0:   #relaunch TORCS every 3 episode because of the memory leak error
    ...
    for j in range(100000):
        ...
        # receive the sensors input s_t and make prediction
        ...
        # add noise to imporve ε-greedy strategy
        ...
        #add to replay buffer,reduce oscillation
        buff.add(s_t, a_t[0], r_t, s_t1, done)
        # Do the batch update
        batch = buff.getBatch(BATCH_SIZE)
        #random small batch extraction
        ...
        if(train_indicator):
            ...
            loss = criterion_critic(y_t, q_values)  #loss function
